...
void cameraPath( in float t, out vec3 ro, out vec3 ta, out vec3 up ) {
    ...
    // show frontside
    camint( ro, t, 5., vec3(810. , 6300. , 1350. ), pro, dro );
    camint( ta, t, 5., vec3(    0. ,    0. ,   0. ), pta, dta );
    camint( up, t, 5., vec3(    0. ,    0.1,   1. ), pup, dup ); 
    
    // go to backside
    camint( ro, t, 5., vec3(-3440. ,-800. , 400. ), pro, dro );
    camint( ta, t, 5., vec3(    0. ,    0. ,   0. ), pta, dta );
    camint( up, t, 7., vec3(    0. ,    0.1,   1. ), pup, dup ); 
    
    // show backside
    camint( ro, t, 7., vec3(-4300. , -1000. , 500. ), pro, dro );
...
// black bands
    vec2 bandy = vec2(0.0,1.0);
    if( uv.y < bandy.x || uv.y > bandy.y ) {
        col = vec3(0.);
    } else {
        // camera
        vec3 ro, ta, up;
        cameraPath( iTime*1, ro, ta, up );
