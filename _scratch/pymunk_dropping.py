...
class MyGame(arcade.Window):
    ...
    def __init__(self, width, height, title):
        ...
        body = pymunk.Body(body_type=pymunk.Body.STATIC)
        shape = pymunk.Segment(body, [SCREEN_WIDTH - 10, 10], [SCREEN_WIDTH, SCREEN_HEIGHT], 0.0)
        shape.friction = 10
        shape.elasticity = 1
        self.space.add(shape, body)
        self.static_lines.append(shape)
        ...
        body = pymunk.Body(body_type=pymunk.Body.STATIC)
        shape = pymunk.Segment(body, [10, 10], [0, SCREEN_HEIGHT], 0.0)
        shape.friction = 10
        shape.elasticity = 1
        self.space.add(shape, body)
        self.static_lines.append(shape)
    ...
    def on_draw(self):
        ...
        # Display timings
        output = f"Processing time: {self.time:.3f}"
        ...
        ball_count = f"Ball count: {len(self.ball_list)}"
        arcade.draw_text(ball_count, 20, SCREEN_HEIGHT - 60, arcade.color.WHITE, 12)
        
        score = f'Score: {self.score}'
        arcade.draw_text(score, 20, SCREEN_HEIGHT - 80, arcade.color.WHITE, 12)
